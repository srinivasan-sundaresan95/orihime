...
import orihime.java_extractor  # noqa: F401 — side-effect: registers JavaExtractor
import orihime.js_extractor  # noqa: F401 — side-effect: registers JsExtractor
...
class _Writer:
    ...
    remote write-serialization server, depending on whether ORIHIME_SERVER_URL
    is set.
...
def _build_constant_index(work_items: list[tuple]) -> dict[str, str]:
    ...
    from orihime.language import get_parser as _get_parser
    import orihime.java_extractor as _jex
...
def _parse_file(args: tuple) -> ParseResult:
    ...
    from pathlib import Path as _Path
    from orihime.language import get_parser as _get_parser, get_extractor as _get_extractor
    # Trigger extractor registration in the child process
    import orihime.java_extractor  # noqa: F401
    import orihime.kotlin_extractor  # noqa: F401
    import orihime.js_extractor  # noqa: F401
...
def index_repo(
    ...
    # Server mode: when ORIHIME_SERVER_URL is set, writes go through the
    ...
    _server_url = os.environ.get("ORIHIME_SERVER_URL", "")
    ...
    if not _has_tables(conn):
        ...
        from orihime.schema import _NODE_TABLES, _REL_TABLES, _DROP_REL_TABLES, _DROP_NODE_TABLES
