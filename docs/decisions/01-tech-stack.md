...
Python is the implementation language for Orihime. The choice is driven by the ecosystem:
...
KuzuDB provides:
- Embedded mode (single file, `~/.orihime/orihime.db`) — always available, no server needed for local dev
...
|------|--------|----------|
| Local embedded | `KUZU_LOCAL_PATH=~/.orihime/orihime.db` | Developer workstation, always available |
