...
2. Create `orihime/<language>_extractor.py` implementing `LanguageExtractor`
3. Call `register(<LanguageExtractor>())` at module bottom
4. Import the module in `orihime/__init__.py` to trigger registration
...
- Third-party contributors can implement an extractor without importing from `orihime`
